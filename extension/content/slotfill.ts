  ...
  switch (intent) {
    case "ads":
      const adSubject = text
        .replace(/^(compose|write|create|make)\s+(me\s+)?a\s+(facebook\s+post|instagram\s+post|social\s+media\s+post|ad)\s+about\s+/i, "")
      ...
      if (lowerText.includes("facebook")) {
        return createStructuredPrompt(
          adSubject,
          "Facebook Post",
          { role: "Social Media Marketing Specialist", voice: "Engaging, conversational" },
          `Create a compelling Facebook post about ${adSubject} that drives engagement and action.`,
          {
            ...
            constraints: "Facebook algorithm optimization, authentic engagement, clear CTA"
          },
          [
          ...
          { format: "Facebook Post", sections: "Hook → Value → CTA", length: "1-2 sentences + hashtags" },
          [
      ...
      if (lowerText.includes("instagram")) {
        return createStructuredPrompt(
          ...
          `Create an engaging Instagram post about ${adSubject} with strong visual appeal.`,
          {
            ...
            constraints: "Visual storytelling, Instagram aesthetics, story format"
          },
          [
    ...
    case "copy.product":
      const product = text
        .replace(/^(write|create|make|compose)\s+(me\s+)?a\s+(product\s+)?(description|copy)\s+(for|about)\s+/i, "")
      ...
      return createStructuredPrompt(
        ...
        { format: "Product Description", sections: "Hook → Features → Benefits → CTA", length: "150-300 words" },
        [
    ...
    case "code.fix":
      ...
      return createStructuredPrompt(
        ...
        `Debug and resolve the technical issue: ${issue}`,
        {
          ...
          constraints: "Minimal changes, clear explanation, maintainable solution"
        },
        [
        ...
        { format: "Technical Solution", sections: "Problem → Analysis → Solution → Prevention" },
        [
    ...
    case "translate":
      ...
      return createStructuredPrompt(
        translateText,
        "Translation",
        { role: "Professional Translator", voice: "Accurate, culturally aware" },
    ...
    case "summarize":
      const summarySubject = text.replace(/^(summarize|summarise|brief|overview)\s+/i, "");
      ...
      return createStructuredPrompt(
        ...
        { format: "Summary", sections: "Overview → Key Points → Conclusions", length: "200-400 words" },
        [
    ...
    case "writing":
      ...
      if (lowerText.includes("essay")) {
        return createStructuredPrompt(
          ...
          { format: "Academic Essay", sections: "Introduction → Background → Analysis → Perspectives → Conclusion", length: "800-1200 words" },
          [
      ...
      if (lowerText.includes("article") || lowerText.includes("blog")) {
        return createStructuredPrompt(
          ...
          `Write an engaging article about ${content} that educates and entertains readers.`,
          {
            ...
            constraints: "Accessible language, engaging storytelling, practical value"
          },
          [
          ...
          { format: "Article", sections: "Hook → Information → Examples → Insights", length: "600-1000 words" },
          [
      ...
      if (lowerText.includes("report")) {
        return createStructuredPrompt(
          ...
          `Compile a comprehensive report on ${content} with actionable insights and recommendations.`,
          {
            ...
            constraints: "Data-driven, objective analysis, actionable recommendations"
          },
          [
          ...
          { format: "Business Report", sections: "Executive Summary → Findings → Analysis → Recommendations", length: "1000-1500 words" },
          [
